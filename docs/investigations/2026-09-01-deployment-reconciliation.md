...
Client mod enforcement remains gated on external `@Apex_cfg` release configuration. It must provide the exact Boolean `QS_missionConfig_clientWorkshopModEnforcementEnabled = true`, plus arrays for `QS_missionConfig_allowedClientWorkshopIds` and `QS_missionConfig_allowedClientModHashes`. At least one list must be non-empty; each is limited to 256 canonical values. IDs are decimal Workshop-ID strings; full hashes are 40-character hexadecimal strings, compared case-insensitively and stored lowercase by convention. Never allowlist `da39a3ee5e6b4b0d3255bfef95601890afd80709`: that empty-payload SHA-1 can be reported by unrelated unsigned/local mods. Version and hash this non-secret configuration, verify it is loaded before client reports, and deploy it atomically with the matching client reporter and server validator. Missing, collectively empty, malformed, or oversized configuration now disables enforcement and logs the configuration error instead of mass-kicking clients. This remains a cooperative client-report policy, not an anti-cheat boundary.

The September 3 deployment expanded the report tuple from `[Workshop ID,name]` to `[Workshop ID,name,full hash]` and allowed either credential. The corrected implementation preserves that behavior while treating a malformed or unavailable per-mod hash as an unusable credential rather than invalidating the complete report. An allowed Workshop ID therefore still succeeds without a usable hash. Hash entries should be reserved for deliberate hash-only exceptions; duplicating every ID-backed mod as a hash would let a removed Workshop ID remain authorized by a stale hash. Client reporter, server validator, and the external configuration must be staged and deployed as one release.
...
Implementation: `627677a` preserves that deployed policy at the exact reconstruction point. Corrective commit `2982ecc` makes enforcement default-off and fail-safe; resolves the reporting player and binds delayed action to UID plus owner; permits one handled report; bounds each allowlist to 256 and reports to 128 entries; requires canonical decimal Workshop IDs and canonical full hashes; sanitizes/bounds displayed data; and limits logged and displayed rejections. The September 3 deployment then introduced three-field ID/name/hash reports and ID-or-hash approval. The follow-up correction preserves that interface, rejects the shared empty-payload SHA-1 as configuration, and makes an invalid per-entry hash unable to override either the ID check or the rest of the report. Enforcement must remain off until `@Apex_cfg` supplies versioned ID and hash arrays plus the exact Boolean enable flag, all three are verified loaded before reports, and allowed/disallowed/malformed/suppressed-client cases pass staging. Even then, client self-reporting is cooperative policy rather than an anti-cheat control.
