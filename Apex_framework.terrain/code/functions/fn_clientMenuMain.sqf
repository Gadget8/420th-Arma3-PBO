...
if (_type isEqualTo 'B7') exitWith {
	if (!(((toLowerANSI (player getVariable ['QS_unit_role',''])) isEqualTo 'donator') || {((getPlayerUID player) in (['DONATOR'] call (missionNamespace getVariable 'QS_fnc_whitelist')))})) exitWith {};
	closeDialog 2;
	['onUnload'] call (missionNamespace getVariable 'QS_fnc_clientMenuMain');
	0 spawn {
		uiSleep 0.1;
