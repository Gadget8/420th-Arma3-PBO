...
if (_rejectRequest) exitWith {
	if (localNamespace getVariable ['QS_diag_rpcLog',FALSE]) then {[_type,_rxID,_isRxJ,FALSE,_rejectReason] call QS_fnc_diagRpc;};
	if (_clientToServer) then {
		private _rateMap = missionNamespace getVariable ['QS_remoteExecCmd_rateMap',createHashMap];
		private _rateKey = str _rxID;
...
if (localNamespace getVariable ['QS_diag_rpcLog',FALSE]) then {[_type,_rxID,_isRxJ,(!_rejectRequest),_rejectReason] call QS_fnc_diagRpc;};
if (_rejectRequest) exitWith {
	private _rateMap = missionNamespace getVariable ['QS_remoteExecCmd_rateMap',createHashMap];
	private _rateKey = str _rxID;
