...
if (_type isEqualTo 'WORLD') then {
	...
	for '_x' from 0 to _attempts step _step do {
		if (diag_tickTime > _timeout) exitWith {
			if (localNamespace getVariable ['QS_diag_safePos',FALSE]) then {[_timeout,_forceFind,(if (isNil '_fnc_scriptNameParent') then {'unknown'} else {_fnc_scriptNameParent})] call QS_fnc_diagSafePos;};
