...
for '_x' from 0 to 1 step 0 do {
	if (localNamespace getVariable ['QS_diag_loopTiming',FALSE]) then {['core.main'] call QS_fnc_diagLoop;};
	_timeNow = time;
	_QS_diagTickTimeNow = diag_tickTime;
	...
	if (_timeNow > _vRespawn_checkDelay) then {
		if (localNamespace getVariable ['QS_diag_loopTiming',FALSE]) then {['core.vehicleMonitor'] call QS_fnc_diagLoop;};
		...
		if (localNamespace getVariable ['QS_diag_loopTiming',FALSE]) then {['core.vehicleMonitor',(count (serverNamespace getVariable ['QS_v_Monitor',[]]))] call QS_fnc_diagLoop;};
		_vRespawn_checkDelay = time + _vRespawn_delay;
	};
	if (_timeNow > _QS_cleanup_checkDelay) then {
		...
		if (_QS_garbageCollector_enabled) then {
			...
			if ((missionNamespace getVariable 'QS_garbageCollector') isNotEqualTo []) then {
				...
				if (localNamespace getVariable ['QS_diag_loopTiming',FALSE]) then {['core.garbage'] call QS_fnc_diagLoop;};
				_QS_garbageCollector = [];
				{
				...
				if (localNamespace getVariable ['QS_diag_loopTiming',FALSE]) then {['core.garbage',(count (missionNamespace getVariable ['QS_garbageCollector',[]]))] call QS_fnc_diagLoop;};
	...
	if (localNamespace getVariable ['QS_diag_loopTiming',FALSE]) then {['core.main',(count allUnits)] call QS_fnc_diagLoop;};
	sleep 3;
};
